...
def get_current_branch():
    output = subprocess.check_output(["git rev-parse --abbrev-ref HEAD"], shell=True).decode('utf8').replace('\n', '')
...
def checkout_remote_branch(branch):
    try:
        ...
        try:
            subprocess.check_output(["git checkout -b %s origin/%s" % (branch, branch)], shell=True)
        except subprocess.CalledProcessError:
            return False
...
def get_tag_branch(tag_name):
    try:
        output = subprocess.check_output(["git branch --contains %s" % tag_name], shell=True).decode('utf8')
        return output
    except subprocess.CalledProcessError:
        return None
